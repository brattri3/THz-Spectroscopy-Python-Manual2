...
\begin{document}
	...
	\begin{table}[H]
		...
		\begin{tabularx}{\textwidth}{lX}
			...
			Systematic angular offset $\theta_{\text{offset}}$\footnotemark[2] & $-0.45^\circ \pm 0.08^\circ$ / $+0.96^\circ \pm 0.06^\circ$ (configuration dependent) \\
			Model approximation error (RMSE)\footnotemark[3] & $\pm 1.39$ dB (dynamic range up to $-45$ dB) \\
			\bottomrule
		\end{tabularx}
	\end{table}
	...
	\footnotetext[2]{\scriptsize The systematic angular offset is configuration-dependent: $-0.45^\circ \pm 0.08^\circ$ is the rotator alignment backlash offset when rotating the first polarizer, and $+0.96^\circ \pm 0.06^\circ$ is the offset when rotating the analyzer (second polarizer).}
	\footnotetext[3]{\scriptsize The 2D residuals analysis shows that the model approximation error is dominated by unmodeled multiple inter-grid reflections and diffraction scattering at large angles ($\theta > 50^\circ$), whereas for small angles ($\theta < 45^\circ$) the local fit RMSE is under $\pm 0.1$ dB.}
