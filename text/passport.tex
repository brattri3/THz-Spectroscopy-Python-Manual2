...
\begin{document}
	...
	% Two-column layout: Sections 3+4 on left, Figure 1 on right
	\noindent
	\begin{minipage}[t]{0.56\textwidth}
		...
		{\small Depending on the receiver type, the power transmission $T(\theta, \nu)$ is calculated via the Blanco model\textsuperscript{*}, where $t_{\perp}(\nu)$, $t_{\parallel}(\nu)$ are complex amplitude transmission coefficients, $\theta_{\text{offset}}$ is the systematic angular offset, and $T_{\text{noise}}$ is the noise floor:
		\begin{itemize}[leftmargin=0.5cm, itemsep=0pt, parsep=0pt, topsep=1pt]
			\item \textbf{Scenario A} (Bolometer / Golay Cell --- polarization-insensitive): \\
			$T = |t_{\perp}|^2 \cos^2(\theta - \theta_{\text{offset}}) + |t_{\parallel}|^2 \sin^2(\theta - \theta_{\text{offset}}) + T_{\text{noise}}$
			\item \textbf{Scenario B} (PCA Menlo Tera K8 --- polarization-sensitive): \\
			$T = \left| t_{\perp} \cos^2(\theta - \theta_{\text{offset}}) + t_{\parallel} \sin^2(\theta - \theta_{\text{offset}}) \right|^2 + T_{\text{noise}}$
		\end{itemize}}
	\end{minipage}
	\hfill
	\begin{minipage}[t]{0.40\textwidth}
		...
		\vspace{-0.6em}
		\captionof{figure}{Experimental setup scheme with polarizer angle $\theta_1$ and analyzer angle $\theta_2$.}
		\label{fig:scheme}
	\end{minipage}
