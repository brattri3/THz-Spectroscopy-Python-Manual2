...
\begin{document}
	...
	\begin{minipage}[t]{0.56\textwidth}
		...
		Depending on the receiver type, the power transmission $T(\theta, \nu)$ is calculated via the Blanco model\textsuperscript{*}:
		\begin{enumerate}[leftmargin=0.6cm, label=\Alph*., itemsep=0pt, parsep=0pt, topsep=2pt]
			\item \textbf{Scenario A} (Bolometer / Golay Cell --- polarization-insensitive): \\
			$T = |t_{\perp}|^2 \cos^2(\theta - \theta_{\text{off}}) + |t_{\parallel}|^2 \sin^2(\theta - \theta_{\text{off}}) + T_{\text{n}}$
			\item \textbf{Scenario B} (PCA Menlo Tera K8 --- polarization-sensitive): \\
			$T = \left| t_{\perp} \cos^2(\theta - \theta_{\text{off}}) + t_{\parallel} \sin^2(\theta - \theta_{\text{off}}) \right|^2 + T_{\text{n}}$
		\end{enumerate}
		{\footnotesize where $t_{\perp}(\nu)$, $t_{\parallel}(\nu)$ are amplitude transmission coefficients; $\theta_{\text{off}}\equiv\theta_{\text{offset}}$; $T_{\text{n}}\equiv T_{\text{noise}}$.}
	\end{minipage}
	\hfill
