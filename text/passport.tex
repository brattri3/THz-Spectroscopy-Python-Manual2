...
\begin{document}
	...
	\begin{table}[H]
		...
		\end{tabularx}
	\end{table}
	\footnotetext{The Blanco model [1] maps a cylindrical wire grid to an equivalent flat strip grid. Electrodynamically, a round wire of nominal diameter $D = 11.0$~$\mu$m interacts with THz radiation similarly to a flat metal strip of width $d \approx D/2$, resulting in a smaller effective electrodynamic width $D_{\text{eff}} \approx 5.66$~$\mu$m.}
	...
	Depending on the receiver type, the introduced power transmission $T(\theta, \nu)$ at frequency $\nu$ is calculated according to the electrodynamic Blanco model [1] using one of the following scenarios:
	\begin{enumerate}[leftmargin=0.8cm, label=\Alph*., itemsep=1pt]
		...
		where $|t_{\perp}(\nu)|^2$ and $|t_{\parallel}(\nu)|^2$ are the power transmission coefficients of the grids for perpendicular and parallel polarizations, respectively, calculated via the Blanco model [1].
		...
		where $t_{\perp}(\nu)$ and $t_{\parallel}(\nu)$ are the complex amplitude transmission coefficients calculated via the Blanco model [1].
	\end{enumerate}
